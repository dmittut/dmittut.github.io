    ...
    (() => {
            ...
            document.addEventListener("DOMContentLoaded", (function() {
                document.querySelectorAll(".button-opt, .button-fop").forEach((button => {
                    button.addEventListener("click", (() => {
                        const isOpt = button.classList.contains("button-opt");
                        const registerToShow = isOpt ? ".opt-register" : ".fop-register";
                        const registerToHide = ".main-register";
                        const formToShow = document.querySelector(registerToShow);
                        if (formToShow) {
                            formToShow.removeAttribute("hidden");
                            const steps = formToShow.querySelectorAll(".step:not([id])");
                            steps.forEach((step => step.removeAttribute("hidden")));
                        }
                        document.querySelector(registerToHide)?.setAttribute("hidden", "true");
                    }));
                }));
                document.querySelectorAll(".button-form-prev").forEach((button => {
                    button.addEventListener("click", (() => {
                        const mainRegister = document.querySelector(".main-register");
                        if (mainRegister) mainRegister.removeAttribute("hidden");
                        document.querySelectorAll(".opt-register, .fop-register").forEach((form => {
                            form.setAttribute("hidden", "true");
                            const steps = form.querySelectorAll(".step");
                            steps.forEach((step => {
                                if (!step.id) step.setAttribute("hidden", "true");
                            }));
                        }));
                    }));
                }));
                document.querySelectorAll("[data-form-next-step]").forEach((button => {
                    button.addEventListener("click", (() => {
                        const currentStep = button.closest(".step");
                        const nextStepSelector = button.getAttribute("data-form-next-step");
                        const nextStep = document.querySelector(nextStepSelector);
                        if (!currentStep || !nextStep) {
                            console.error("Не удалось найти текущий или следующий шаг.", {
                                currentStep,
                                nextStepSelector
                            });
                            return;
                        }
                        const hasErrors = formValidate.getErrors(currentStep);
                        if (hasErrors === 0) {
        ...
        function initSliders() {
            if (document.querySelector(".swiper")) {
                new Swiper(".partners-main__slider", {
                    modules: [ Navigation ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 3,
                ...
                new Swiper(".slide-certificates__slider", {
                ...
                new Swiper(".sto-images__slider--one", {
                    modules: [ Navigation ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 1,
                ...
                new Swiper(".sto-images__slider--two", {
                    modules: [ Navigation ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 1,
